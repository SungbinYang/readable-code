...
public class MinesweeperGame {
    ...
    private static final Cell[][] BOARD = new Cell[BOARD_ROW_SIZE][BOARD_COL_SIZE]; // 게임판
    ...
    private static void initializeGame() {
        ...
        for (int i = 0; i < LAND_MINE_COUNT; i++) {
            ...
            BOARD[row][col].turnOnLandMine(); // 지뢰 10개 랜덤 생성
        }
    ...
    private static void open(int row, int col) {
        ...
        if (BOARD[row][col].isOpened()) { // 이미 열린 셀인지
            return;
        }
        ...
        if (BOARD[row][col].hasLandMineCount()) { // 지뢰숫자를 가졌는지
            return;
        }
